...
std::priority_queue<game*,std::vector<game*>,game::compare> q;
std::vector<game*> visited;

void astar(){
    q = std::priority_queue<game*,std::vector<game*>,game::compare>(game::compare());
    visited.clear();
...
int search( int bound){
    ...
    int min = NOT_FOUND;
    for(game* succ : moves)
         if (std::find_if(visited.begin(), visited.end(), [succ] (const game* s) { return memcmp(&(s->field),&(succ->field),sizeof(int)*N*N) == 0; }) == visited.end()){
